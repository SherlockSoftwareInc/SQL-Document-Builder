...
namespace OctofyPro.SchemaData
{
    public sealed class DBSchema
    {
        ...
        public bool HasObjects => _tables.Count > 0 || _views.Count > 0 || _functions.Count > 0;
        ...
        public async Task<bool> LoadAllColumnsAsync(CancellationToken token = default)
        {
            ...
            {
                await using var provider = await ConnectAsync(_connection, token);
                var allColumns = await provider.GetAllColumnsAsync(string.Empty, token);

                var lookup = allColumns
                    .GroupBy(c => $"{ReadSchema(c)}|{ReadObjectName(c)}", StringComparer.OrdinalIgnoreCase)
                ...
                foreach (var item in DataObjects)
                {
                    ...
                    var key = $"{item.SchemaName}|{item.ObjectName}";
                    if (lookup.TryGetValue(key, out var rawColumns))
                    {
                        var ordinal = 1;
        ...
        private static string ReadObjectName(object source) =>
            ReadStringProperty(source, "ObjectName", "Name", "TableName", "ViewName", "FunctionName");
